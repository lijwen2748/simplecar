...
*/
 ...
 namespace car {
    //state 
 	...
 	{
 	public:
 	    State (const Assignment& latches, Model* model = NULL) : s_ (latches), pre_ (NULL), next_ (NULL) {
 	    if (model != NULL)
		    reorder (model, false);
		}

 		State (const State *s, const Assignment& inputs, const Assignment& latches, const bool forward, Model* m = NULL, const bool last = false); 
 		
 		State (State *s): pre_ (s->pre_), next_(s->next_), s_(s->s_), inputs_(s->inputs_), last_inputs_(s->last_inputs_), 
 		init_ (s->init_), id_ (s->id_), dep_ (s->dep_), reorder_s_ (s->reorder_s_) {}

 		~State () {}
 		
 		void reorder (Model*, bool);
		...
		inline void work_count_inc () {work_count_ ++;}
 		...
 		bool operator== (const State &st) const {
 		    if (s_.size () != st.s_.size ()) return false;
